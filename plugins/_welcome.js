...
handler.before = async function (m, { conn, groupMetadata }) {
  ...
  const fecha = new Date().toLocaleDateString('es-ES', { timeZone: 'America/Santo_Dominio', day: 'numeric', month: 'long', year: 'numeric' })
  const desc = groupMetadata.desc?.toString() || 'Sin descripción'

  // --- FKONTAK PARA EL QUOTED ---
  const fkontak = {
  ...
  // --- LÓGICA DE BIENVENIDA (TEXTO ORIGINAL) ---
  if (m.messageStubType == WAMessageStubType.GROUP_PARTICIPANT_ADD) {
    ...
    await conn.sendMessage(m.chat, {
      product: {
        productImage: { url: welcomeApi },
        productId: 'welcome-shadow',
        title: '─ W E L C O M E ─🥷🏻',
        currencyCode: 'USD',
        priceAmount1000: '0',
        retailerId: 1677,
  ...
  // --- LÓGICA DE DESPEDIDA (TEXTO ORIGINAL) ---
  if (m.messageStubType == WAMessageStubType.GROUP_PARTICIPANT_REMOVE || m.messageStubType == WAMessageStubType.GROUP_PARTICIPANT_LEAVE) {
    ...
    await conn.sendMessage(m.chat, {
      product: {
        productImage: { url: goodbyeApi },
        productId: 'goodbye-shadow',
        title: '─Ａ Ｄ 𝐈 Ō S─👋🏻',
        currencyCode: 'USD',
        priceAmount1000: '0',
        retailerId: 1677,
